/**
 * @file FILEPATH: /home/developer/node_socket/appX.js
 * @description Applicazione Node.js che utilizza 
 * il framework Express e la libreria Socket.io 
 * per creare un'applicazione di chat in tempo reale.
 */
...
async function eseguiOperazioneConMutex(socketMappa,titolo,msqemit) {
  ...
  try {
    ...
    await socketMulticastMutex (socketMappa,titolo,msqemit);
...
// Utilizzo del mutex in una funzione asincrona
async function socketMulticast(socketMappa,titolo,msqemit) {
    await eseguiOperazioneConMutex(socketMappa,titolo,msqemit);
...
const updatedChat = htmlChat.replace('__SERVER_ADDRESS__',  serverIpAddress +':3000');
...
//        socket.broadcast.emit('new_client', {username:socket.username,classe:socket.classe,real:socket.real, punti:0});
    ...
    socket.on('message', function (message) {
        ...
        if (condizione ) {    
          ...
          // POSIZIONATO CORRETTAMENTE gara.get(socket.classe)[socket.livello].delete(myid);
           ...
           //     xnumgara.get(socket.classe).push(Math.pow(2,socket.livello)+Math.floor(Math.random()*Math.pow(2,socket.livello)));
            ...
            gara.get(socket.classe)[socket.livello].set( myid,socket); //aggiungo vincitore 
            if (socket.livello<=saltaLiv[0]){
                let tipConv = socket.livello%6 ;
                if (tipConv==0)  //d2b
      ...
      //  socket.emit('message', {liv: socket.livello ,username: socket.username,real:socket.real ,classe: socket.classe , punti: 2*socket.punti-socket.prove , message: message}); 
       ...
       console.log('Aggiorno iscritti ');
        ...
        socketMulticast(gara.get(socket.classe)[--i],'message', {liv: socket.livello ,username: socket.username,real: socket.real,classe: socket.classe , punti: (2*socket.punti-socket.prove) , message: message});
    ...
    socket.on('disconnect', function() {
            ...
            let punteggio= (2*socket.punti-socket.prove)/4;
            console.log(punteggio)
            //message += '&#128169;'.repeat(socket.prove);
          //  socketMulticast(gara.get(socket.classe),'message', {username: socket.username,real: socket.real, punti: '&#127942;'.repeat(socket.punti), 
          //  message: 'Si ritira \u25BA '+socket.livello+' con punti:'+punteggio+'&#128169;'.repeat(socket.prove)});
            let  newUser = { 
                ...
                prove: socket.prove,
                classe: socket.classe ,
                VotoPer4: (2*socket.punti-socket.prove),
                gara: gare
            };
            fs.readFile('users.json', 'utf8', (err, data) => {
                ...
                users.sort((a, b) => b.VotoPer4 - a.VotoPer4);
            ...
            fs.writeFile('config.json', JSON.stringify({userSimbol : userSimbol, gara : gare+1 }) , function(err) {
                if (err) {
                    ...
                    console.log('Dati scritti correttamente nel file config ');
                }
            });
        }
        });
